    ...
    for (i = 0; i < strlen(barlayout); i++) {
        ...
        case 'n':
            ...
            if (m->sel) {
                ...
                drw_text(drw, x, 0, moveright ? tw : m->ww, bh, lrpad / 2,
                         m->sel->name, 0);
                if (m->sel->isfloating)
                    drw_rect(drw, x + boxs, boxs, boxw, boxw, m->sel->isfixed,
                             0);
            } else {
                drw_setscheme(drw, scheme[SchemeNorm]);
        ...
        case 's':
            if (m == selmon) { /* status is only drawn on selected monitor */
                drw_setscheme(drw, scheme[SchemeNorm]);
                tw = drawstatusbar(selmon, bh, stext);
                tw -= 2 * sp; /* 2px right padding */
                if (moveright)
                    x -= tw;
            }
            break;
        ...
        case 't':
            ...
            for (j = 0; j < LENGTH(tags); j++) {
                ...
                if (ulineall || m->tagset[m->seltags] &
                                    1 << j) /* if there are conflicts, just
                                    move these
